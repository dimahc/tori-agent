...
export async function trigger_ci_check(
  ...
  try {
    ...
    await recordCheckResult(runtimePaths, workflowRunId, `check:ci:${config.check_id}`, CHECK_STATUS.passed, `CI check passed: ${reviewCheck.command}`, CHECK_POLICY.blocking);
    return { success: true, stdout, stderr, exitCode: 0, durationMs: Date.now() - started };
  } catch (error) {
    const err = error as { stdout?: string; stderr?: string; code?: number; message: string };
    await recordCheckResult(runtimePaths, workflowRunId, `check:ci:${config.check_id}`, CHECK_STATUS.failed, err.stderr ?? err.message, CHECK_POLICY.blocking);
    return {
      success: false,
      stdout: err.stdout ?? "",
